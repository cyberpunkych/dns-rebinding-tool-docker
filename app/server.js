...
dnsServer.on('query', query => {
  ...
  if(domain == 'dnsrebindtool.'+process.env.DOMAIN){
   let record
   record = new dns.ARecord(process.env.DOMAIN_IP)
   query.addAnswer(domain, record, 0)
   dnsServer.send(query)
  }

  if(domain.indexOf(process.env.DOMAIN) < 0)
    return
